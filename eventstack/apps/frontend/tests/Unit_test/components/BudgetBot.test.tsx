...
describe("components/BudgetBot", () => {
  it("opens the panel and sends a quick prompt", async () => {
    ...
    await user.click(view.getByRole("button", { name: /Ask Budget Bot/i }))
    ...
    const [eventId, payload] = apiMock.askBudgetBot.mock.calls[0]
    expect(eventId).toBe("evt-1")
    expect(payload?.prompt).toContain(quickPrompt)
    expect(payload?.context).toContain("Gala Night")
  ...
  it("submits manual questions and clears the input", async () => {
    ...
    await waitFor(() => expect(apiMock.askBudgetBot).toHaveBeenCalledTimes(1))
    const [_evt, payload] = apiMock.askBudgetBot.mock.calls[0]
    expect(_evt).toBe("evt-1")
    expect(payload?.prompt).toContain("Check variance status")
