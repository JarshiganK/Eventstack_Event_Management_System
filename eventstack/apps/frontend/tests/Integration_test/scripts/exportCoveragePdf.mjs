...
async function generatePdf() {
  const summaryPath = resolve(process.cwd(), "coverage", "integration", "coverage-summary.json");
  if (!existsSync(summaryPath)) {
    throw new Error(
      `Integration coverage summary not found at ${summaryPath}. Run "npm run testi" first.`,
    );
  }
  ...
  const html = buildHtml(summary, "Frontend Integration Test Coverage", relativePath(summaryPath));
  ...
  try {
    ...
    const outputPath = join(reportsDir, "coverage-integration.pdf");
    await page.pdf({
      path: outputPath,
      format: "A4",
    ...
    console.log(`Saved integration coverage PDF to ${outputPath}`);
  } finally {
    await browser.close();
  }
}
