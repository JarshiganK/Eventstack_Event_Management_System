...
export async function createVenue(v: { name: string; address?: string; lat: number; lng: number; zone?: string; subzone?: string }) {
  return http<{ id: string }>(
    '/admin/venues', { method: 'POST', body: JSON.stringify(v) }
  )
}
export async function updateVenue(id: string, v: { name: string; address?: string; lat: number; lng: number; zone?: string; subzone?: string }) {
  return http<{ id: string }>(
