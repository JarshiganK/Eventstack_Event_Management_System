...
describe("lib/api/misc", () => {
  ...
  it("handles bookmark mutations and notifications", async () => {
    httpMock.mockResolvedValue({ ok: true });
    const { addBookmark, removeBookmark, notifications } = await import(
      "../../../src/lib/api/misc.js"
    );
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: vi.fn().mockResolvedValue([{ id: "n1" }]),
      text: vi.fn(),
    });
    const result = await notifications("user-1");
