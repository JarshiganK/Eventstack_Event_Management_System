...
export default async function searchRoutes(app) {
    ...
    app.get("/search", async (req) => {
        ...
        const { rows } = await query(`SELECT e.*, (SELECT url FROM event_images i WHERE i.event_id=e.id ORDER BY ord ASC LIMIT 1) as cover_url
       FROM events e
       WHERE ${where}
       ORDER BY e.starts_at ASC`, params);
