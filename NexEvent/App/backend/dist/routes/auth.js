...
export default async function authRoutes(app) {
    ...
    app.post("/auth/register", async (req, reply) => {
        ...
        const role = normalizedRole === "ORGANIZER" ? "ORGANIZER" : "USER";
        const id = cuid();
        const passwordHash = await hashPassword(password);
        try {
            await query("INSERT INTO users (id, email, password_hash, role) VALUES ($1,$2,$3,$4)", [id, email, passwordHash, role]);
        }
        catch (e) {
            if (e?.code === "23505") {
    ...
    app.post("/auth/login", async (req, reply) => {
        ...
        if (body.email === HARD_CODED_ADMIN.email && body.password === HARD_CODED_ADMIN.password) {
            const user = { id: HARD_CODED_ADMIN.id, email: HARD_CODED_ADMIN.email, role: "ADMIN" };
            const token = signJwt(user);
            return { token, user };
        }
